...
#' @param allmax_forced list of numeric with min and max of colorscheme
#'
#' @return
#' @export
#'
...
STACYmap <- function(gridlyr = NULL, 
  ...
  if(!is.null(allmax_forced)){
    lims <- list(grid = allmax_forced, pt = allmax_forced)
  }
    ...
    if (!is.null(ptlyr_size_var) & !is.null(ptlyr_shape_var)) {
      ...
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   ...
                   alpha = I(0.8),  shape = 21, stroke = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_STROKE,
                   show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE)) + 
        scale_size_manual(values = c(GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE, GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE / 2), 
                          guide = guide_legend(ptlyr_size_var, nrow = 2, direction = 'horizontal', title.vjust = GLOBAL_STACY_OPTIONS$GLOBAL_LEG_TITLE_VJUST, order = 3))
      ...
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   ...
                   alpha = I(0.8), size = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE, stroke = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_STROKE,
                   show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE)) + 
        scale_shape_manual(values = c(23, 22, 24, 21, 0:20), 
      ...
      map_plot <- map_plot + 
        geom_point(data = ptlyr,
                   ...
                   alpha = I(0.8), shape = 21, size = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_SIZE, stroke = GLOBAL_STACY_OPTIONS$GLOBAL_POINT_STROKE,
                   show.legend = c(fill = {if (is.null(gridlyr) | splcol) {TRUE} else {FALSE}}, size = TRUE))
    }
